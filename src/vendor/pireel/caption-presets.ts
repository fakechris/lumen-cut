// Ported from pireel/pireel (AGPL-3.0), packages/studio-engine/src/caption-presets.ts
// lumen-cut notes:
// - `name` values are pireel's i18n catalog keys; lumen-cut renders display names
//   via CAPTION_PRESET_NAMES in src/views/editor/captionPresets.ts instead.
// - lumen-cut keeps its own default look: no preset is selected until the user
//   picks one (DEFAULT_CAPTION_PRESET is kept for parity but not applied here).
